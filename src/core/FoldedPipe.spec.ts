import FoldedPipe from "./FoldedPipe";

describe('FoldedPipe Tests', () => {
    it('should push/backward/forward correctly', function () {
        let foldedPipe = new FoldedPipe<string>({});
        foldedPipe.push('1');
        foldedPipe.push('2');
    ...
    it('should drop oldest item when stack items over maxLength', function () {
        const maxLength = 20;
        const foldedPipe = new FoldedPipe<number>({maxLength: maxLength});
        for (let i = 0; i < 21; i++) {
            foldedPipe.push(i);
        }
